...
export default function JaraiPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (currentPage === 0) {
          const storedData = localStorage.getItem(`jaraiData_${language}`);
          if (storedData) {
        ...
        const result = await fetchPostsByCategory({ category: 'JRAI', language, page: currentPage, size: 5 });
        setData(result);
        setTotalPages(result.totalPages || 0);
        
        // Store in localStorage for future use (only for page 0)
        if (currentPage === 0) {
          localStorage.setItem(`jaraiData_${language}`, JSON.stringify(result));
        }
      } catch (error) {
      ...
      <main className="main-content" style={{ 
        ...
        <div style={{ 
          ...
          <div style={{
            ...
            <span style={{ color: '#333', fontWeight: '500' }}>{language === 'en' ? 'Jrai Longhouse' : 'Nhà dài Gia rai'}</span>
          </div>
          {data.content.map((p) => (
